...
class UnrealModel(object):
  ...
  def _acktr_base_loss(self):
    ...
    #base_loss = policy_loss + value_loss
    train_loss = policy_loss + self.vf_coef*vf_loss

    #constructing the fisher loss 
    self.pg_fisher = pg_fisher_loss = -tf.reduce_mean(self.base_adv)
    sample_net = self.base_v + tf.random_normal(tf.shape(self.base_v))
    self.vf_fisher = vf_fisher_loss = - vf_fisher_coef*tf.reduce_mean(tf.pow(self.base_v - tf.stop_gradient(sample_net), 2))
    self.joint_fisher = joint_fisher_loss = pg_fisher_loss + vf_fisher_loss

    self.params=params = find_trainable_variables("model")
    self.grads_check = grads = tf.gradients(train_loss,params)

    return joint_fisher_loss,params,grads
  ...
  def prepare_loss_acktr(self):
    with tf.device(self._device):
      self.fisher_loss, self.params,self.grads = self._acktr_base_loss()
      ...
      intermediate_loss = vr_loss + pc_loss + rp_loss 
